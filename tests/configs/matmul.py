...
def make_matmul_op_configs():
    ...
    with OperationTestConfig.module_name("matmul"):
        ...
        # Batched dot_general with multiple free dimensions per operand.
        ...
        # Batched scalar product: batch dims only, no contracting or free dims.
        # This pattern is generated by sparse BCOO operations.
        yield OperationTestConfig(
            lambda x, y: lax.dot_general(x, y, dimension_numbers=(([], []), ([0], [0]))),
            lambda key: random.normal(key, (5,)),
            lambda key: random.normal(key, (5,)),
            name="batched_scalar_product",
        )

        # Batched outer product: batch dims + free dims, no contracting dims.
        yield OperationTestConfig(
            lambda x, y: lax.dot_general(x, y, dimension_numbers=(([], []), ([0], [0]))),
            lambda key: random.normal(key, (3, 4)),
            lambda key: random.normal(key, (3, 5)),
            name="batched_outer_product",
        )
